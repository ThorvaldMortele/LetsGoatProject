...
namespace CrazyGames
{
    ...
    public class CrazySDK : Singleton<CrazySDK>
    {
        ...
        private readonly List<Action<UserInfo>> getUserInfoCallbacks = new List<Action<UserInfo>>();
        ...
        private void Start()
        {
            ...
            {
                gameLink = "https://www.crazygames.com/game/yourFabulousGameHere", locale = "en-US",
                userInfo =
                    new UserInfo
                    {
                        countryCode = "demo", browser = new Software {name = "demo", version = "demo"}, os = new Software {name = "demo", version = "demo"}
                    }
            };
            CallInitCallbacks();
        ...
        {
            getUserInfoCallbacks.ForEach(cb => { cb?.Invoke(initObj.userInfo); });
            getUserInfoCallbacks.Clear();
        }
        ...
        public IEnumerator GetScreenshotRoutine()
        {
            ...
            tex.Apply();
            var jpgImg = tex.EncodeToJPG();
            wrapGFFeature(() => { GetScreenshotSDK(jpgImg, jpgImg.Length); });
        ...
        public void GetUserInfo(Action<UserInfo> callback)
        {
            if (initObj != null)
            {
                callback(initObj.userInfo);
            }
            else
            {
                getUserInfoCallbacks.Add(callback);
            }
        }
